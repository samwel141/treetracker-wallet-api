...
class ActionTokenService {
  ...
  */
  ...
  // Page through the wallet's tokens collecting ones not already promised by
  // another outstanding link, until `count` are found or the wallet runs out.
  async _selectAvailableTokens(walletLoginId, count, reservedTokenIds) {
    const selected = [];
